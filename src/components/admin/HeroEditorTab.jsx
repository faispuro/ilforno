...
export const HeroEditorTab = ({ heroData, setHeroData, onSave }) => {
  ...
  const validateHero = (data) => {
    const errs = {};
    const hasImage = data.bgImagePreview || data.bgImage;

    if (!hasImage) errs.bgImage = 'La imagen de portada es obligatoria.';
  ...
  const handleHeroBgImageChange = (file) => {
    ...
    setHeroData(updated);
    if (heroTouched.bgImage) {
      const errs = validateHero(updated);
      setHeroErrors((prev) => ({ ...prev, bgImage: errs.bgImage }));
    }
  };
  ...
  const handleSaveHero = async () => {
    ...
    await submitWithToast(
      async () => {
        ...
        if (onSave) {
          await onSave('hero', heroData);
        }
      },
      {
        successMessage: '¡Portada guardada con éxito!',
      ...
      <div className="block-in relative bg-stone-900/40 border border-stone-800/90 p-6 sm:p-8 rounded-3xl space-y-7 backdrop-blur-sm overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between border-b border-stone-800/80 pb-5">
          ...
          <button
            ...
            className="group relative w-11 h-11 bg-stone-100 hover:bg-white text-stone-950 rounded-2xl flex items-center justify-center cursor-pointer transition-all duration-300 shadow-lg shadow-black/40 hover:scale-105 active:scale-95 disabled:opacity-70 disabled:pointer-events-none shrink-0"
          >
            {isSaving ? <Loader2 className="w-5 h-5 animate-spin text-stone-950" /> : <Save className="w-5 h-5 transition-transform group-hover:rotate-6 text-stone-950" />}
          </button>
        </div>

        <fieldset disabled={isSaving} className="space-y-6 border-none p-0 m-0 disabled:opacity-60 disabled:pointer-events-none">
          <div className="relative z-10 space-y-2">
            <label className="text-[11px] font-mono uppercase tracking-widest text-stone-400 font-semibold flex items-center justify-between">
              <span>Imagen de Fondo Portada <span className="text-amber-500">*</span></span>
              <span className="text-[10px] text-stone-500 normal-case font-normal">Recomendado 1920x1080 (JPG/WEBP)</span>
            </label>

            <div className={`relative w-full h-48 rounded-2xl border-2 border-dashed bg-stone-950/60 overflow-hidden group transition-colors flex items-center justify-center ${
              heroErrors.bgImage ? 'border-red-500/80' : 'border-stone-800 hover:border-amber-600/50'
            }`}>
              {currentHeroBg ? (
                <>
            ...
            <FieldError message={heroErrors.bgImage} />
          </div>
